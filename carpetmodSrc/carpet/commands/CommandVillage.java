...
public class CommandVillage extends CommandCarpetBase {
    ...
    public static final String USAGE_ADD = "/village <overworld|nether|end|*> add <index> <x> <y> <z>";
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        switch (args[1].toLowerCase(Locale.ENGLISH)) {
            ...
            case "remove": // Removes villages
                ...
                break;
            case "add": // Add door to village
                if (args.length != 6) throw new WrongUsageException(USAGE_ADD);

                int index = parseInt(args[2]);
                BlockPos pos = parseBlockPos(sender, args, 3, false);
                try {
                    VillageCollection collection = worlds.get(0).getVillageCollection();
                    int tick = collection.getTickCounter();
                    collection.getVillageList().get(index).addVillageDoorInfo(new VillageDoorInfo(pos, EnumFacing.NORTH, tick));
                    notifyCommandListener(sender, this, String.format("Door added to village %d", index));
                    break;
                } catch (IndexOutOfBoundsException e) {
                    throw new NumberInvalidException("commands.generic.num.invalid", args[2]);
                }
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, BlockPos targetPos) {
        ...
        switch (args.length) {
            ...
            case 2:
                return getListOfStringsMatchingLastWord(args, "order", "undo", "move", "centers", "load", "remove", "add");
            case 3:
